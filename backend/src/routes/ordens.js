...
router.get("/filters/options", async (req, res) => {
  try {
    ...
    const anos = [...new Set(data.map(item => item.ano_servico).filter(Boolean))].sort((a, b) => a - b); // Ordenar anos numericamente
    ...
    res.json({
      ...
      anos // Adicionado anos ao retorno
    });
...
router.put("/:id/classificar", async (req, res) => {
  try {
    ...
    if (defeito_subsubgrupo) updateData.defeito_subsubsubgrupo = defeito_subsubgrupo;
