...
// Rota de debug para verificar dados salvos
router.get('/debug', async (req, res) => {
  try {
    ...
    // Contar registros por mês/ano
    const contagemPorMes = {};
    ordens.forEach(os => {
      const chave = `${os.ano_servico}-${os.mes_servico}`;
      contagemPorMes[chave] = (contagemPorMes[chave] || 0) + 1;
    });

    res.json({
      totalRegistros: ordens.length,
      ultimosRegistros: ordens,
      contagemPorMes: contagemPorMes,
      exemploRegistro: ordens[0] || null
    });
...
// Rota para corrigir dados existentes
router.post('/fix-data', async (req, res) => {
  try {
    const { fixExistingData } = require('../scripts/fixExistingData');
    ...
    res.status(500).json({ error: 'Erro interno do servidor na correção de dados' });
  }
});
